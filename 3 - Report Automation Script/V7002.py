...
sql_file_reader = open('v7002_sql.sql', 'r', encoding='utf8')
...
V7002_1 = pd.DataFrame()
V7002_2_4 = pd.DataFrame()
V7002_5 = pd.DataFrame()
V7002_6 = pd.DataFrame()

var_list = [V7002_1, V7002_2_4, V7002_5, V7002_6]
...
# Create unique symbols from V7002_6
currency = set()

for symbol in V7002_6['currency']: 
    currency.add(symbol)
...
# Merge 
V7002_6 = V7002_6.merge(df, on = ['currency'])

# Create converted tables
V7002_6['BYN'] = round(V7002_6.sum_amount * (V7002_6.cur_officialrate / V7002_6.cur_scale), 2)

V7002_6['USD'] = round(V7002_6.BYN / list(dict(
    requests.get("https://www.nbrb.by/api/exrates/rates/usd?parammode=2", verify=False).json()).values())[-1], 2)

# Feature enginnering
V7002_6['multiplier'] = V7002_6['instrument_group'].apply(lambda x: 20 if str(x) == '4.0' else (20 if str(x) == 'nan' else 100))

V7002_6['feature_usd'] = V7002_6['USD'] * V7002_6['multiplier']
V7002_6['feature_byn'] = V7002_6['feature_usd'] * usd_curr
V7002_6.drop(columns = ['multiplier', 'instrument_group', 'size_of_the_contract'], inplace = True)

V7002_6.full_symbol = V7002_6.full_symbol.apply(lambda x : str(x).replace('#',''))

#================ DATAFRAMES TO EXCEL ================

def dataframes_to_excel(df_list, sheets, file_name, spaces): 
    writer = pd.ExcelWriter(file_name,engine='xlsxwriter')  
    data = V7002_6.to_excel(writer, sheet_name = 'Лист 1', index = False)
    row = 0 
...
dataframes = [V7002_1, V7002_2_4, V7002_5]  
